...
export default function FacultyScheduleScreen() {
  ...
  const anchor = data?.calendarAnchor ?? { year: today.getFullYear(), month: today.getMonth() };
        ...
        {tab === 'calendar' ? (
          ...
          <WalkthroughTarget id={WALKTHROUGH_TARGETS.FACULTY.SCHEDULE_CALENDAR} style={styles.weeklyWrap}>
            <View style={styles.weeklyHeader}>
              <View>
                <Text style={styles.weeklyTitle}>{data?.weekly.title ?? 'Weekly Schedule'}</Text>
                <Text style={styles.weeklySubtitle}>
                  Week {currentWeek?.weekNumber ?? weekIndex + 1}
                  {data?.weekly.semesterLabel ? `, ${data.weekly.semesterLabel}` : ''}
                </Text>
              </View>
